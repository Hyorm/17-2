...
class MovieRatingData 
{
	...
	accRatingsOfMovies = new TreeMap<Integer, Double>() ;
        ...
        TreeMap<Integer, Double>
        ratingsOfMovies = new TreeMap<Integer, Double>() ;
	...
	load (FileReader f) throws IOException {
		for (CSVRecord r : CSVFormat.newFormat(',').parse(f)) {
			...
			if (numRatingsOfMovies.containsKey(movie) == false) {
				...
				ratingsOfMovies.put(movie, rating);
			}
			else {
	...
	void showRatingStat() {
		...
		*/
		...
		double [] ratings = new double[ ratingsOfMovies.keySet().size()] ;
	
		int i = 0;
		for (Integer movie : ratingsOfMovies.keySet()) {
		        ratings[i] = ratingsOfMovies.get(movie);
			i++;
		}
		
		HistogramDataset dataset = new HistogramDataset() ;
		dataset.setType(HistogramType.RELATIVE_FREQUENCY) ;
		dataset.addSeries("Histogram", ratings, 9) ;
		JFreeChart chart = ChartFactory.createHistogram("Num. Avg Ratings",
		        "Avg Rating", "Num", dataset, PlotOrientation.VERTICAL, false, false, false) ;
		JPanel panel = new ChartPanel(chart) ;
		frame.setContentPane(panel) ;
		frame.pack() ;
		frame.setVisible(true) ;
